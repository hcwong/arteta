...
6. **Two-column homepage layout** — ✅ delivered (post-MVP). Width-gated at ≥140 cols: `viewListTwoCol` in `internal/tui/tui.go` joins a fixed-width (60-col) workflow list with a bordered preview pane that mirrors the selected workflow's `tmux capture-pane -ep -t arteta-<name>.0`. Refresh: `tea.Tick(1s)` while dwelling, plus immediate refresh on cursor move. Below 140 cols the homepage falls back to the original single-column layout. Implementation notes worth remembering:
   - **No persistent state** — capture-pane is called on demand. We never run `pipe-pane` and never write a session log file. Trade: we lose pre-capture scrollback, but adopting `pipe-pane` would mean per-workflow lifecycle bookkeeping (start hook, log rotation, cleanup on Close/Revive) that didn't pay off for the glance-level use case.
   - **Lossy by design** — preview lines are ANSI-truncated (`truncateANSI` in `helpers.go`) to fit the column. tmux freezes the pane width at the last attached client's width, so the source can be much wider than the preview; truncating dodges syncing widths. Strict wrapping was rejected because Claude's right-aligned input border wraps awkwardly.
   - **ANSI assumption** — `tmux capture-pane -e` emits only SGR (colour/bold), never cursor-control. If we ever switch to `capture-pane -p` of a non-tmux source, that assumption breaks and `truncateANSI` needs a fuller CSI walker.
   - **Last-good-frame** — transient capture errors (race with KillSession, brief tmux hiccup) keep the previous frame on screen; persistent failures (≥5 in a row) escalate to the existing `m.err` footer. Avoids flicker without hiding real outages.
   - **Tick runs in all modes** — the 1Hz tick keeps firing even with Create/Confirm/Help modals up; we just don't render the preview unless `mode == ModeList`. Stopping/restarting the tick on every mode transition was fiddlier than one extra `tmux capture-pane` per second.
